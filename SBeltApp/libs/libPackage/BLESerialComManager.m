...
#define BONDING_DEVICE @"bondingDev"
...
-(void)saveBondingDevice:(NSUUID*)identifier
{

  [[NSUserDefaults standardUserDefaults] setObject:identifier.UUIDString forKey:BONDING_DEVICE];
  [[NSUserDefaults standardUserDefaults] synchronize];
}
-(void)delBondingDevice
{
  [[NSUserDefaults standardUserDefaults] removeObjectForKey:BONDING_DEVICE];
  [[NSUserDefaults standardUserDefaults] synchronize];
}
-(NSUUID*)getBondingDevIdentifier
{
  NSString *uuidString = [[NSUserDefaults standardUserDefaults] objectForKey:BONDING_DEVICE];
...
-(resultCodeType)startEnumeratePorts:(float)timeout{
  ...
  //用户重新枚举，如果之前已经绑定过设备，则意味用户不想连接之前绑定的设备
  [self delBondingDevice];
    if (CM) {
      
      
        NSArray *services = [NSArray arrayWithObjects:[CBUUID UUIDWithString:STR_SERVICE_ID_SERIAL_PORT], nil];
        NSArray *ConnectedPerArray = [CM retrieveConnectedPeripheralsWithServices:services];
        if(ConnectedPerArray && ConnectedPerArray.count)
          [CM cancelPeripheralConnection:ConnectedPerArray.firstObject];
...
-(void)startDiscoverSerialPortServiceForPort:(BLEPort *)port{
    ...
    if (port) {
        if (port.state == STATE_OPENING && port.peripheral.isConnected) {
            NSArray *services = [NSArray arrayWithObjects:[CBUUID UUIDWithString:STR_SERVICE_ID_SERIAL_PORT], nil];
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
  ...
  //自动连接
  if([portInPorts.peripheral.identifier isEqual:[self getBondingDevIdentifier]])
  {
    paramsPackage4Open param;
    [self startOpen:portInPorts withParams:param];
  }
}
-(void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral{
   BLEPort *port = [self getPortByPeripheral:peripheral];
    ...
    if (port == nil) {
      ...
      [self saveBondingDevice:peripheral.identifier];
      [CM stopScan];
    }
